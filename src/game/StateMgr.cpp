...
UnitAction* UnitStateMgr::CreateStandartState(UnitActionId stateId, ...)
{
    va_list vargs;
    ...
    switch (stateId)
    {
        ...
        case UNIT_ACTION_ONVEHICLE:
        {
            ...
            state = new OnVehicleState(param);
            break;
        }
...
{
    UnitAction* state = CreateStandartState(actionId);
...
void UnitStateMgr::PushAction(UnitActionId actionId, UnitAction* state)
{
    PushAction(actionId, state, staticActionInfo[actionId].priority, staticActionInfo[actionId].restoreable);
}

void UnitStateMgr::PushAction(UnitActionId actionId, UnitAction* state, UnitActionPriority priority, eActionType restoreable)
{
    ActionInfo* oldInfo = CurrentState();
...
bool ActionInfo::operator == (UnitAction* action)
{

...
void ObjectMgr::LoadCreatures()
{
    ...
    do
    {
        ...
        if (data.spawnMask & ~spawnMasks[data.mapid])
            sLog.outErrorDb("Table `creature` have creature (GUID: %u Entry: %u) that have wrong spawn mask %u including not supported difficulty modes for map (Id: %u).", guid, entry, data.spawnMask, data.mapid);
...
void ObjectMgr::LoadPlayerInfo()
{
    ...
    // Fill gaps and check integrity
    for (int race = RACE_HUMAN; race < MAX_RACES; ++race)
    {
        // skip nonexistent races
        if(!((1 << (race-1)) & RACEMASK_ALL_PLAYABLE) || !sChrRacesStore.LookupEntry(race))
            continue;

        for (int class_ = CLASS_WARRIOR; class_ < MAX_CLASSES; ++class_)
        {
            // skip nonexistent classes
...
bool PlayerCondition::Meets(Player const* player, Map const* map, WorldObject const* source, ConditionSource conditionSourceType) const
{
    ...
    switch (m_condition)
    {
        ...
        {
            if (!map)
                map = player ? player->GetMap() : source ? source->GetMap() : nullptr;

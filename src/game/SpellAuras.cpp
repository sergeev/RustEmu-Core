...
Aura* SpellAuraHolder::CreateAura(SpellEntry const* spellproto, SpellEffectIndex eff, int32* currentBasePoints, SpellAuraHolder* holder, Unit *target, Unit *caster, Item* castItem)
{
...
Aura* SpellAuraHolder::CreateAura(AuraClassType type, SpellEffectIndex eff, int32* currentBasePoints, SpellAuraHolder* holder, Unit* target, Unit* caster, Item* castItem)
{
...
{
     SpellAuraHolder* holderPtr = new SpellAuraHolder(spellproto, target, caster, castItem);
     return holderPtr;
}
            ...
            if (!targets.empty())
            {
                for (GuidSet::const_iterator tIter = targets.begin(); tIter != targets.end(); tIter++)
                {
                    ...
                    if (SpellEntry const *actualSpellInfo = sSpellMgr.SelectAuraRankForLevel(GetSpellProto(), i_target->getLevel()))
                    {
                        ...
                        SpellAuraHolder* holder = i_target->GetSpellAuraHolder(actualSpellInfo->Id, GetAffectiveCasterGuid());

                        if (!holder || holder->IsDeleted())
                        {
                            SpellAuraHolder* newholder = CreateSpellAuraHolder(actualSpellInfo, i_target,  GetAffectiveCaster());
                            newholder->SetAuraDuration(GetAuraDuration());
                            /*Aura* aura = */newholder->CreateAura(AURA_CLASS_AREA_AURA, m_effIndex, &actualBasePoints, newholder, i_target, GetAffectiveCaster(), nullptr);
                        ...
                        else
                        {
                            ...
                            {
                                Aura* aura = holder->CreateAura(AURA_CLASS_AREA_AURA, m_effIndex, &actualBasePoints, holder, i_target, GetAffectiveCaster(), nullptr);
...
//                    case 47015: break;
                    ...
                    case 59376:
                    {
                        if (target)
                        {
                            ...
                            target->CastCustomSpell(target, 51132, &healthMissing, nullptr, nullptr, true);
                        }
                        return;
                    }
//                    // Party G.R.E.N.A.D.E.
...
//                    case 67546: break;
                    ...
                    // Ice Tomb
                    case 70157:
                        if (uint32(time(nullptr) - GetAuraApplyTime()) >= 20 && !triggerTarget->HasAura(71665))
    ...
    else                                                    // initial triggeredSpellInfo != nullptr
    {
        ...
        switch(auraId)
        {
            ...
            case 6474:
            {
                ...
                for(Unit::AuraList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
                {
                    if ((*itr)->GetSpellProto()->GetSpellIconID() == 2289 && (*itr)->GetSpellProto()->SpellFamilyName == SPELLFAMILY_SHAMAN)
                    {
                        ...
                        target->CastSpell(target, 59566, true, nullptr, this);
                        break;
                    }
                 }
                break;
            ...
            case 48094:                                      // Intense Cold
                triggerTarget->CastSpell(triggerTarget, trigger_spell_id, true, nullptr, this);
                return;
...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
                ...
                switch(GetId())
                {
                    ...
                    case 68645:
                        ...
                        if (target->GetTypeId() == TYPEID_PLAYER)
                        {
                            ...
                            target->CastSpell(target, 69192, true, nullptr, this);
                            // Rocket Pack - causing damage
                            target->CastSpell(target, 69193, true, nullptr, this);
                            return;
                        }
                        return;
    ...
    else
    {
        ...
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_MAGE && GetSpellProto()->GetSpellFamilyFlags().test<CF_MAGE_LIVING_BOMB>())
        {
            ...
            {
                if (this->GetCaster())
                    GetCaster()->CastSpell(target,m_modifier.m_amount,true,nullptr,this);
                else
            ...
            switch(GetId())
            {
                case 6495:                                  // Sentry Totem
                {
                    ...
                    if (totem && apply)
                        ...
                        ((Player*)target)->SetViewPoint(nullptr);
    ...
    switch (form)
    {
        ...
        case FORM_MOONKIN:
        {
            ...
            for (Unit::AuraList::const_iterator iter = slowingAuras.begin(); iter != slowingAuras.end(); ++iter)
            {
                ...
                SpellAuraHolder* holder = iter->GetHolder();
                if (!holder || holder->IsDeleted())
                    continue;
...
void Aura::HandleBindSight(bool apply, bool /*Real*/)
{
    ...
    if (apply)
        ...
        ((Player*)caster)->SetViewPoint(nullptr);
}
...
void Aura::HandleAuraModStun(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        if (target->hasUnitState(UNIT_STAT_MELEE_ATTACKING))
            target->SendMeleeAttackStop(nullptr);
    ...
    else
    {
        ...
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_HUNTER && GetSpellProto()->GetSpellFamilyFlags().test<CF_HUNTER_WYVERN_STING2>())
        {
            ...
            caster->CastSpell(target, spell_id, true, nullptr, this);
            return;
        }
    }
}
    ...
    if (!apply)
    {
        switch(GetId())
        {
            ...
            case 66083:                                     // Lightning Arrows (Trial of the Champion encounter)
                ...
                {
                    if (Unit* pCaster = GetCaster())
                        pCaster->CastSpell(pCaster, 66085, true, nullptr, this);
                }
        ...
        case 71265:                                     // Swarming Shadows (Queen Lana'thel)
        {
            if (apply)
            {
                ...
                if (Unit *pCaster = GetCaster())
                {
                    ...
                    SpellAuraHolder* holder = pCaster->GetSpellAuraHolder(70877);
                    if (!holder)
...
void Aura::HandlePeriodicDamage(bool apply, bool Real)
{
    ...
    else
    {
        ...
        if (spellProto->Id == 41917)
            ...
            target->CastSpell(target, 74607, true, nullptr, nullptr, GetCasterGuid());
        else if (spellProto->Id == 74792) // SPELL_SOUL_CONSUMPTION - Ruby sanctum boss Halion
            target->CastSpell(target, 74799, true, nullptr, nullptr, GetCasterGuid());
        // Void Shifted
        else if (spellProto->Id == 54361 || spellProto->Id == 59743)
            target->CastSpell(target, 54343, true, nullptr, nullptr, GetCasterGuid());
    }
    ...
    switch (m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            switch (GetId())
            {
                ...
                {
                    target->CastSpell(target, 74567, true, nullptr, nullptr, GetCasterGuid());
                    break;
                }
                case 74792: // SPELL_SOUL_CONSUMPTION - Ruby sanctum boss Halion, added mark (74795, dummy) every tick
                {
                    target->CastSpell(target, 74795, true, nullptr, nullptr, GetCasterGuid());
                    break;
                }
                case 67297:
                case 65950:
                    ...
                    if (pCaster->GetPet())
                    {
                        ...
                        if (!groupPetsCopy.empty())
                        {
                            for (GuidSet::const_iterator itr = groupPetsCopy.begin(); itr != groupPetsCopy.end(); ++itr)
                            {
                                ...
                                {
                                    if (pPet->isAlive())
                                        pCaster->CastCustomSpell(pPet, 32554, &gain_amount, nullptr, nullptr, true, nullptr, nullptr, pCaster->GetObjectGuid());
                                }
                            }
                        }
                    }
...
//              case 50530: break;
                ...
                case 62717:                                 // Slag Pot
                {
                    target->CastSpell(target, 65722, true, nullptr, this);
                ...
                {
                    target->CastSpell(target, 65723, true, nullptr, this);

                    // cast Slag Imbued if the target survives up to the last tick
                    if (GetAuraTicks() == 10)
                        target->CastSpell(target, 62836, true, nullptr, this);
                    return;
                }
                ...
                case 68646:
                {
                    ...
                    target->CastCustomSpell(target, 66240, &damage, nullptr, nullptr, true, nullptr, this);
                    if (Unit* caster = GetCaster())
                        target->CastCustomSpell(caster, 66125, &heal, nullptr, nullptr, true, nullptr, this);
                    return;
                }
...
void Aura::HandleAuraLinked(bool apply, bool Real)
{
    ...
    if (apply)
    {
            ...
            spellInfo->HasAttribute(SPELL_ATTR_HIDE_IN_COMBAT_LOG))
        {
            ...
            pTarget->CastCustomSpell(pTarget, spellInfo, &bp0, &bp1, &bp2, true, nullptr, this, GetCasterGuid(), GetSpellProto());
            ...
            (GetSpellProto()->GetSpellIconID() == 264 || GetSpellProto()->GetSpellIconID() == 1933))
        {
            ...
            if (pCaster)
                pCaster->CastCustomSpell(pTarget, spellInfo, &bp0, nullptr, nullptr, true, nullptr, this, GetCasterGuid(), GetSpellProto());
        }
        else
            pTarget->CastSpell(pTarget, spellInfo, true, nullptr, this);
    }
    else
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            else if (!apply && m_spellProto->GetSpellFamilyFlags().test<CF_MAGE_ARCANE_MISSILES_CHANNEL>())
            {
                if (Unit * caster = GetCaster())
                {
                    ...
                    if (SpellAuraHolder* holder = caster->GetSpellAuraHolder(44401, caster->GetObjectGuid()))
        ...
        case SPELLFAMILY_PALADIN:
        {
            if (m_spellProto->Id == 19746)                  // Aura Mastery (on Concentration Aura remove and apply)
            {
                ...
                if (apply && caster->HasAura(31821))
                    caster->CastSpell(caster, 64364, true, nullptr);
                else if (!apply)
            ...
            switch(GetId())
            {
                ...
                case 73787:
                {
                    if (apply)
                    {
                        ...
                        if (Unit *caster = GetCaster())
                        {
                            if (SpellAuraHolder* holder = caster->GetSpellAuraHolder(GetId()))
                ...
                if (m_target->GetObjectGuid().IsPet())
                {
                    if (!apply)
                        if (Unit* owner = ((Pet*)m_target)->GetOwner())
                            if (SpellAuraHolder* holder = owner->GetSpellAuraHolder(m_spellProto->Id))
...
{
    SpellAuraHolder* currentHolder = nullptr;
    if (!remove)
    {
        for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i)
        {

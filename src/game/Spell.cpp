...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    if (m_healing)
    {
        ...
        if (crit)
        {
            ...
            damageInfo.damage  = caster->SpellCriticalHealingBonus(m_spellInfo, damageInfo.damage, nullptr);
        }
        else
...
void Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask)
{
    ...
    for(int effectNumber = 0; effectNumber < MAX_EFFECT_INDEX; ++effectNumber)
    {
        if (effectMask & (1 << effectNumber))
        {
            HandleEffects(unit, nullptr, nullptr, SpellEffectIndex(effectNumber));
        }
    }
...
void Spell::update(uint32 difftime)
{
        ...
        (m_spellInfo->Effect[EFFECT_INDEX_0] != SPELL_EFFECT_STUCK || !m_caster->m_movementInfo.HasMovementFlag(MOVEFLAG_FALLINGFAR)))
    {
        ...
        if (m_spellState == SPELL_STATE_CASTING && !m_spellInfo->HasAttribute(SPELL_ATTR_EX5_CAST_CHANNEL_WHEN_MOVEING))
            cancel();
                ...
                if (ihit->missCondition == SPELL_MISS_NONE)
                {
                    ...
                    if (unit && unit->isAlive())
                    {
                        ...
                        if (roll_chance_i(chance))
                            caster->CastSpell(unit, auraSpellInfo->EffectTriggerSpell[auraSpellIdx], true, nullptr, (*i)());
                    }
                }
            }
        }
...
void Spell::SendChannelUpdate(uint32 time)
{
    if (time == 0)
    {
        ...
        {
            if (/*DynamicObject* dynObj = */m_caster->GetDynObject(m_triggeredByAuraSpell ? m_triggeredByAuraSpell->Id : m_spellInfo->Id))
                ((Player*)m_caster)->SetViewPoint(nullptr);
        }
        ...
        switch(m_spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                if (Unit* target = m_targets.getUnitTarget())
                {
                    ...
                    bool result = pathFinder.calculate(x, y, z, false);
...
WorldObject* Spell::GetCastingObject() const
{
    if (m_originalCasterGuid.IsGameObject())
        return m_caster->GetMap() ? m_caster->GetMap()->GetGameObject(m_originalCasterGuid) : nullptr;
    else
        return m_caster;
}
...
bool Spell::FillCustomTargetMap(SpellEffectIndex i, UnitList& targetUnitMap, uint8& effToIndex)
{
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 73710:
        {
            ...
            if (SpellAuraHolder* holder = m_caster->GetSpellAuraHolder(72756))
                radius += holder->GetStackAmount() * 1.0f;
            // 25man normal - 5% growth
            else if (SpellAuraHolder* holder = m_caster->GetSpellAuraHolder(74162))
                radius += holder->GetStackAmount() * 0.5f;
            // 10man heroic - 10% growth
            else if (SpellAuraHolder* holder = m_caster->GetSpellAuraHolder(74163))
                radius += holder->GetStackAmount() * 1.0f;
            // 25man heroic - 5% growth
            else if (SpellAuraHolder* holder = m_caster->GetSpellAuraHolder(74164))
                radius += holder->GetStackAmount() * 0.5f;
    ...
    // remove nullptr targets (weird, but...)
    for (UnitList::iterator itr = targetUnitMap.begin(); itr != targetUnitMap.end();)
    {
        if (!*itr)
            itr = targetUnitMap.erase(itr);

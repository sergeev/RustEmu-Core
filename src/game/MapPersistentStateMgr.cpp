...
: m_instanceid(InstanceId), m_mapid(MapId),
  m_difficulty(difficulty), m_usedByMap(nullptr), m_needRemove(false)
{
}
...
void DungeonResetScheduler::LoadResetTimes()
{
    ...
    if(result)
    {
        do
        {
            ...
            if (_oldresettime > uint64(time(nullptr) + INSTANCE_MAX_RESET_OFFSET))
            {
                MapDifficultyEntry const* mapDiff = GetMapDifficultyData(mapid,Difficulty(difficulty));
...
MapPersistentState* MapPersistentStateManager::AddPersistentState(MapEntry const* mapEntry, uint32 instanceId, Difficulty difficulty, time_t resetTime, bool canReset, bool load /*=false*/, bool initPools /*= true*/, uint32 completedEncountersMask /*= 0*/)
{
    ...
    {
        if (!resetTime || resetTime < time(nullptr))
        {
...
void MapPersistentStateManager::CleanupInstances()
{
    ...
    _DelHelper(CharacterDatabase, "id, map, difficulty", "instance", "LEFT JOIN character_instance ON character_instance.instance = id LEFT JOIN group_instance ON group_instance.instance = id WHERE character_instance.instance IS nullptr AND group_instance.instance IS nullptr");

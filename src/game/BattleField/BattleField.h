...
class BFObject
{
    public:
        ...
        virtual void InitFor(PvpTeamIndex teamIdx, bool reset = true);
        bool IsIntact() const;
...
class MANGOS_DLL_SPEC BattleField : public OutdoorPvP
{
    public:
        ...
        virtual void StartBattle(PvpTeamIndex defender);
        virtual void EndBattle(PvpTeamIndex winner, bool byTimer);
        virtual void Update(uint32 diff) override;

        Map* GetMap();
        PvpTeamIndex GetDefender() const { return m_defender; }
        PvpTeamIndex GetAttacker() const { return m_defender == TEAM_INDEX_ALLIANCE ? TEAM_INDEX_HORDE : TEAM_INDEX_ALLIANCE; }

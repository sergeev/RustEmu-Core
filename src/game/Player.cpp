...
Player::Player(WorldSession* session): Unit(), m_mover(this), m_camera(nullptr), m_achievementMgr(this), m_reputationMgr(this)
{
    m_speakTime = 0;
    m_speakCount = 0;
...
{
    return (p == this || (GetGroup() != nullptr && GetGroup()->SameSubGroup(this, p)));
}
...
void Player::GiveXP(uint32 xp, Unit* victim)
{
    ...
    if (GetAccountLinkedState() != STATE_NOT_LINKED  && CheckRAFConditions())
...
void Player::DeleteOldCharacters(uint32 keepDays)
{
    ...
    QueryResult* resultChars = CharacterDatabase.PQuery("SELECT guid, deleteInfos_Account FROM characters WHERE deleteDate IS NOT nullptr AND deleteDate < '" UI64FMTD "'", uint64(time(nullptr) - time_t(keepDays * DAY)));
    if (resultChars)
    {
...
{
    SpellAuraHolder* dPoison = nullptr;
...
void Player::SendUpdatedWorldStates(bool force)
{
    ...
    if (IsBeingTeleported() || GetLastWorldStateUpdateTime() == time(nullptr))
        return;
    ...
    SetLastWorldStateUpdateTime(time(nullptr));
}
...
InventoryResult Player::_CanTakeMoreSimilarItems(uint32 entry, uint32 count, Item* pItem, uint32* no_space_count /*=nullptr*/, uint32* itemLimitCategory /*=nullptr*/) const
{
...
QuestStatusData* Player::GetQuestStatusData(uint32 questId)
{
    ...
    return itr == mQuestStatus.end() ? nullptr : &itr->second;
}
...
//    Map* targetMap = sMapMgr.FindMap(savedLocation.GetMapId(), state ? state->GetInstanceId() : 0);
    ...
    else
    {
        ...
        else if (mapEntry->IsDungeon())
        {
            ...
            if (at)
            {
                if (state && time(nullptr) - time_t(fields[22].GetUInt64()) < 30)
                {
                    SetLocationMapId(savedLocation.GetMapId());
                    Relocate(savedLocation);
                }
    ...
    if (result)
    {
        do
        {
            ...
            for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i)
            {
                ...
                Aura* aura = holder->CreateAura(spellproto, SpellEffectIndex(i), nullptr, holder, this, nullptr, nullptr);
    ...
    if (IsInWorld())
    {
        ...
        else
        {
            ...
            if (GetSpellDuration(spellInfo) > 0)
                if (uint64(time(nullptr)) - GetSpellDuration(spellInfo) / IN_MILLISECONDS > _saveTime)
                    return;
...
void Player::SendModifyCooldown(uint32 spell_id, int32 delta)
{
    ...
    AddSpellCooldown(spell_id, 0, uint32(time(nullptr) + int32(result / IN_MILLISECONDS)));
...
{
    Unit* combotarget = pet ? pet->GetMap()->GetUnit(targetGuid) : nullptr;
    if (pet && combotarget)
    {
...
Player* Player::GetNextRandomRaidMember(float radius, bool onlyAlive)
{
    ...
    return nearMembers.empty() ? nullptr : nearMembers[urand(0, nearMembers.size() - 1)];
}
